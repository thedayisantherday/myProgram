...
import com.example.zxg.myprogram.fragment.PullToRefreshFragment;
import com.example.zxg.myprogram.fragment.RecyclerviewFragment;
import com.example.zxg.myprogram.fragment.WheelviewFragment;
...
//        banner_view.startAutoPlay(1000);
    ...
    private void initFragments(){
        ...
        mTabTitles = new String[]{ "Recycler View", "Wheel View", "Pull to refresh", "明星名人", "男神", "女神", "音乐",
                "舞蹈", "旅行", "美食", "美妆时尚", "涨姿势", "宝宝", "萌宠乐园", "二次元"};
        mFragments = new ArrayList<Fragment>();
        RecyclerviewFragment recyclerviewFragment = new RecyclerviewFragment();
        mFragments.add(recyclerviewFragment);

        WheelviewFragment wheelviewFragment = new WheelviewFragment();
        mFragments.add(wheelviewFragment);

        PullToRefreshFragment pullToRefreshFragment = new PullToRefreshFragment();
        mFragments.add(pullToRefreshFragment);

        for (int i = 3; i < mTabTitles.length; i++) {
